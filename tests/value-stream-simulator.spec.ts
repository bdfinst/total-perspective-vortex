...
class SimulatorPage {
  ...
  async isSimulationPlaying(): Promise<boolean> {
  ...
  // Metrics
  async getMetric(label: string): Promise<string> {
  ...
  async waitForMetricChange(label: string, initialValue: string, timeout = 5000) {
    const testId = `metric-${label.toLowerCase().replace(/\s+/g, '-')}`;
  ...
  async openStepConfigByIndex(index: number) {
    const step = await this.getStepByIndex(index);
    await step.locator('button:has(svg)').first().click();
    await this.page.waitForTimeout(300);
  }

  async clickStepConfigButton(buttonText: string) {
  ...
  async setStepField(fieldLabel: string, value: string) {
  ...
  async getStepField(fieldLabel: string): Promise<string> {
    const configMenu = this.page.locator('.absolute.z-10').last();
    const label = configMenu.getByText(fieldLabel, { exact: true });
    const input = label.locator('..').locator('input');
  ...
  async countSteps(): Promise<number> {
    return await this.page.locator('div.relative.w-72').count();
  }

  async isStepVisible(stepName: string): Promise<boolean> {
    return await this.page.locator(`text=/${stepName}/i`).isVisible();
  }

  // Work Items
  async countItemsInQueue(stepName: string): Promise<number> {
    const step = await this.getStepByName(stepName);
  ...
  async countItemsInProcess(stepName: string): Promise<number> {
    const step = await this.getStepByName(stepName);
  ...
  async countCompletedItems(): Promise<number> {
  ...
  async hasBottleneckIndicator(stepName: string): Promise<boolean> {
    const step = await this.getStepByName(stepName);
  ...
  async waitForTick(minTick: number, timeout = 10000) {
    await this.page.waitForFunction(
      (min) => {
...
test.describe('Step Configuration', () => {
  ...
  test('should move step left', async ({ page }) => {
    ...
    const developmentBox = await page.locator('div.relative.w-72:has-text("1. Development")').boundingBox();
    const analysisBox = await page.locator('div.relative.w-72:has-text("2. Analysis")').boundingBox();
  ...
  test('should move step right', async ({ page }) => {
    ...
    const developmentBox = await page.locator('div.relative.w-72:has-text("3. Development")').boundingBox();

    ...
    <div className={cn(
      ...
      {item.state === 'PROCESS' && !item.finishedAt && (
        <div className="w-full h-1 overflow-hidden rounded-full bg-slate-200">
          <div
            className={cn("h-full transition-all duration-300", isDefect ? "bg-red-500" : "bg-blue-500")}
      ...
      {item.isRework && (
        <div className="absolute -top-1 -right-1 w-3 h-3 bg-orange-500 text-white rounded-full flex items-center justify-center text-[8px] font-bold shadow-sm z-10">!</div>
      )}
    </div>
  );
}

...
import type { WorkItem, StepConfig, WorkType } from '../types';
...
interface UseSimulationReturn {
  ...
  tick: number;
  createItem: (type: WorkType) => void;
  resetSimulation: () => void;
  setSteps: React.Dispatch<React.SetStateAction<StepConfig[]>>;
}
...
export function useSimulation({
  ...
  const [localSteps, setLocalSteps] = useState<StepConfig[]>(steps);
  ...
  const runTick = useCallback(() => {
    ...
    setLocalSteps(currentSteps => currentSteps.map((step, index) => {
      const queueItems = items.filter(i => i.stepIndex === index && i.state === 'QUEUE');
      const waitTime = queueItems.reduce((acc, item) => acc + (tickRef.current - item.queueEnteredAt), 0);
    ...
    setItems(currentItems => {
      ...
      nextItems.forEach(item => {
        ...
        if (item.state === 'QUEUE') {
          const activeInStep = nextItems.filter(i =>
            i.stepIndex === item.stepIndex && i.state === 'PROCESS' && i.id !== item.id
          ).length;
